...
drawmyplot<-function(geneList, tsne.obj, name){
	for(gene in geneList){
		...
		try(print(Seurat::VlnPlot(tsne.obj, gene, do.return = T, point.size.use = 0, cols.use = my_palette)))
		dev.off()
		png(filename=paste("FeaturePlots/", name, "-Ftr_",gene, ".png", sep=""), width=800, height=800)
		try(Seurat::FeaturePlot(tsne.obj, gene,cols.use=c("grey", "blue"), pt.size = 1, do.return = T))
		dev.off()
	}
}
