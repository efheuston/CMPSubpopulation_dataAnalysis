...
emphasis_plots_PCA<-function(object, file_suffix, iterationSlot, background_color = "black", emphasis_color = "red", ...){
	...
	for(x in 1:length(iteration_vector)){
		...
		png_plotFunction(Seurat::PCAPlot(object = object, cols.use = color_vector, group.by = iterationSlot, pt.size = 1), filename = paste(option_arguments$projectName, "_", unique_id,"_", file_suffix,".png", sep = ""), ...)
	}
}
