...
Transcriptional comparison of 11 populat2ions. Each sample was sequenced in duplicate.  
* LSK  
* CMP  
* GMP  
* CFU-E  
...
We want a list of genes upregulated in CMP subpopulations compared to CMP, regardless of their relative expression level in other populations. Therefore repeat DGE using only these samples and set CMP as intercept
```{r, warning=FALSE}
...
# Convert row names to entrez gene ID
symbolid <- with(mouseIDTable, mouseIDTable$MGI[match(row.names(csvcmp.cpm), mouseIDTable$Entrez.Gene_ID)])
rownames(csvcmp.cpm) <- symbolid
write.table(csvcmp.cpm, file = "avgCPM_forCXvCMP.txt", quote = FALSE, sep = '\t')
```
__Table is written in python__


Estimate dispersion with CMP defined as intercept
